...
# 発送者情報ファイル選択ボタン
sender_info_button = JButton.new("発送者情報を選択")
sender_info_label = javax.swing.JLabel.new("発送者情報が選択されていません")

sender_info_button.add_action_listener do |e|
  ...
  if sender_result == JFileChooser::APPROVE_OPTION
    ...
    sender_info_label.setText("発送者情報が選択されました")
  else
    sender_info_label.setText("発送者情報が選択されていません")
  end
end
...
carrier_button.add_action_listener do |e|
  ...
  if selected_option
    if ["ヤマト", "郵便局（定型便）"].include?(selected_option) && @sender_info.nil?
      JOptionPane.showMessageDialog(nil, "まず発送者情報を選択してください", "エラー", JOptionPane::ERROR_MESSAGE)
...
file_button.add_action_listener do |e|
  ...
  if result == JFileChooser::APPROVE_OPTION
    ...
    when "郵便局（定型便）"
      case @selected_shop
      when "アマゾン"
        convert_amazon_to_japanpost(file, @sender_info)
      else
        JOptionPane.showMessageDialog(nil, "このショップには対応していません。", "エラー", JOptionPane::ERROR_MESSAGE)
      end
    when "クリックポスト"
...
# フレームにコンポーネントを追加
frame.add(sender_info_button)
frame.add(sender_info_label)
